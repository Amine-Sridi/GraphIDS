...
#     # Ground truth label for evaluation (from labeled datasets like NF-UNSW-NB15-v3)
#     ground_truth_label: Optional[int] = Field(
#         default=None, 
#         description="Ground truth label: 0=benign, 1=malicious/anomalous (optional)"
#     )
    
#     # All 51 training features for proper reconstruction error calculation
#     # Optional: if provided, these will be used instead of inferring from basic fields
#     all_features: Optional[dict] = Field(
#         default=None,
#         description="Optional: all 51 NF-UNSW-NB15-v3 features for accurate preprocessing"
#     )
...
#     ground_truth_label: Optional[int] = Field(
#         default=None,
#         description="Ground truth label for evaluation (0=benign, 1=malicious)"
#     )
#     # Flow characteristics for dashboard display
#     protocol: int = Field(default=6, description="Protocol number (6=TCP, 17=UDP, 1=ICMP)")
...
#         default="low",
#         description=(
#             "Human-readable severity tier derived from anomaly score. "
...
#     # Performance metrics (computed when ground truth is available)
#     true_positives: int = Field(default=0, description="Number of correctly identified anomalies")
#     false_positives: int = Field(default=0, description="Number of benign flows incorrectly flagged as anomalous")
#     true_negatives: int = Field(default=0, description="Number of correctly identified benign flows")
#     false_negatives: int = Field(default=0, description="Number of missed anomalies")
    
#     fpr: float = Field(
#         default=0.0, 
#         ge=0.0, 
#         le=1.0,
#         description="False Positive Rate = FP / (FP + TN). Used to trigger retraining."
#     )
#     tpr: float = Field(
#         default=0.0,
#         ge=0.0,
#         le=1.0,
#         description="True Positive Rate (Sensitivity) = TP / (TP + FN)"
#     )
...
#         default=False,
#         description="Whether FPR exceeds the retraining threshold"
#     )
...
#     label: int = Field(..., ge=0, le=1, description="0=benign, 1=anomalous")
#     window_id: int = Field(..., description="Time window ID")
...
#     top_anomaly_sources: List[str] = Field(default_factory=list, description="Top 5 source IPs with anomalies")

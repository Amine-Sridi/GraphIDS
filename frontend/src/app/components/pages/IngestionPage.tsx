...
export function IngestionPage() {
  const { isActive, toggleActive, throughput, totalFlows, ingestionRate, setIngestionRate, aggregatedStats } = useBackend();
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            ...
            <MetricTile
              ...
              label="ANOMALY RATE"
              value={`${((aggregatedStats.anomalyCount / Math.max(1, aggregatedStats.total)) * 100).toFixed(1)}%`}
              unit="of window"
              color={aggregatedStats.anomalyCount > 0 ? '#f85149' : '#7d8590'}
            />
          </div>
        </div>
      </div>
